...
public final class Paper261Provider implements IPlatformProvider {
    ...
    public Optional<IPlatform<?>> tryProvide(Plugin plugin, IListenerBridge bridge) {
        ...
        if (IPlatformProvider.existsClass("org.bukkit.craftbukkit.CraftServer")
            ...
            return Optional.of(Paper261StaticProvider.provide(plugin, bridge));
        }
        return Optional.empty();
    }
}

final class Paper261StaticProvider {

    private Paper261StaticProvider() {
    }

    static IPlatform<?> provide(Plugin plugin, IListenerBridge bridge) {
        return new Paper261Platform(plugin, bridge);
    }
}
